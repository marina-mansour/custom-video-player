...
interface CustomVideoPlayer {
  customObj: { src: string; width: number };
}

class CustomVideoPlayer implements CustomVideoPlayer {
  public customObj;

  constructor(custom: { src: string; width: number }) {
    this.customObj = custom;
    this.createCustomVideo();
  }

  private createCustomVideo(): void {
    //create video
    ...
    volumeSlider.addEventListener("input", () => {
      ...
      if (muteBtn.firstChild instanceof HTMLElement) {
        if (video.muted || video.volume === 0)
          muteBtn.firstChild.classList.replace(
            "fa-volume-high",
            "fa-volume-xmark"
          );
        else
    ...
    //backward
    const backwardBtn = this.createBtn(
      "backward",
    ...
    right?.appendChild(backwardBtn);
    //skip forward
    const forwardBtn = this.createBtn(
      "forward",
    ...
    right?.appendChild(forwardBtn);
    //full screen button
    const fullScreenBtn = this.createBtn(
      "full-screen",
      "fa-solid fa-expand",
      () => {
        try {
          if (!document.fullscreenElement) {
            ...
            if (fullScreenBtn.firstChild instanceof HTMLElement) {
              fullScreenBtn.firstChild.classList.replace(
                "fa-expand",
                "fa-compress"
              );
